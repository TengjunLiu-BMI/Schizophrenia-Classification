...
def train(model, num_epochs=150):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model, '.\model_sample\model -epoch '+str(epoch)+'-k '+str(k) + ' -rand_seed ' + str(rand_seed) + '.pth')
...
for train_idx, val_idx in kf.split(dataPoland):
    ...
    mat_path = '.\matData\SZ Classification Poland sample k ' + str(k) + '_' + str(numK) + ' rand_seed ' + str(rand_seed) + '.mat'
