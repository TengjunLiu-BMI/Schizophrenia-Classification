...
# fileNamePoland = 'dataset_sample_Poland.mat'
...
# tensor_trainX = torch.Tensor(dataPoland).to(device)  # transform to torch tensor
# tensor_trainY = torch.Tensor(labelPoland).to(device)
# train_dataset = data.TensorDataset(tensor_trainX, tensor_trainY)
# data_loader_Poland = data.DataLoader(train_dataset, batch_size=batchSize, shuffle=True)
...
k = 0
acc_ts_baseline_all = np.zeros((numK, 1))
for test_idx, train2_idx in kf.split(dataRussia):
    ...
    acc_ts_baseline_all[k-1] = test(model_CNN, test_loader)

mat_path = '.\matData\SZ Classification Baseline Russia Sample rand_seed ' + str(rand_seed) + '.mat'
data2saveDic = {"acc_ts_baseline_all": acc_ts_baseline_all}
