...
def train(model, num_epochs=150):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model, '.\model_individual\model_FT -epoch '+str(epoch)+'-k '+str(k) + ' -rand_seed ' + str(rand_seed) + '.pth')
...
for test_idx, train2_idx in kf.split(dataRussia):
    ...
    mat_path_pretrained = '.\matData\SZ Classification Poland individual k ' + str(k_Poland) + '_' + str(numK_Poland) + ' rand_seed ' + str(
        rand_seed) + '.mat'
    ...
    model_pth_pretrained = '.\model_individual\model -epoch ' + str(epoch) + '-k ' + str(k_Poland) + ' -rand_seed ' + str(rand_seed) + '.pth'
    model_CNN = torch.load(model_pth_pretrained)
    model_CNN.to(device)

    train_params = [
        # model_CNN.fc1.weight, model_CNN.fc1.bias, # ~52
        # model_CNN.fc2.weight, model_CNN.fc2.bias, # ~56
        # model_CNN.conv1._modules['0'].weight, model_CNN.conv1._modules['0'].bias, # ~57
        model_CNN.conv2._modules['0'].weight, model_CNN.conv2._modules['0'].bias, # 45 -> ~57
        # model_CNN.conv3._modules['0'].weight, model_CNN.conv3._modules['0'].bias # ~56
    ]
    ...
    mat_path = '.\matData\SZ Classification Adaptation Russia individual k ' + str(k) + '_' + str(numK) + ' rand_seed ' + str(rand_seed) + '.mat'
